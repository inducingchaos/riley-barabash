...
const Progress = forwardRef<ElementRef<typeof ProgressPrimitive.Root>, ProgressOptions>(({ className, value, customInternals, children, ...props }, ref) => (
    <ProgressPrimitive.Root ref={ref} className={cn("relative h-2 w-full overflow-hidden rounded-full bg-primary/20", className)} {...props}>
        {customInternals ? children : <ProgressPrimitive.Indicator className={cn("h-full w-full flex-1 bg-primary transition-all")} style={{ transform: `translateX(-${100 - (value ?? 0)}%)` }} />}
...
const ProgressIndicator = forwardRef<ElementRef<typeof ProgressPrimitive.Indicator>, ProgressIndicatorOptions>(({ className, value, ...props }, ref) => <ProgressPrimitive.Indicator ref={ref} className={cn("h-full w-full flex-1 bg-primary transition-all", className)} style={{ transform: `translateX(-${100 - (value ?? 0)}%)` }} {...props} />)
